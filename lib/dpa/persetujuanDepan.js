module.exports = {
    print: async (page) => {
        ...
        const listCetak = await page.evaluate(async ()=>{
            ...
            let response = await fetch(`https://burukab.sipd.kemendagri.go.id/siap/halaman-persetujuan-dpa/tampil-unit/daerah/main/budget/2022/388/0?_=${date}`)
            ...
            for (const skpd of listSKPD) {
                let postJadwal = await fetch(`https://burukab.sipd.kemendagri.go.id/siap/jadwal/2022/hist-jadwal-dpa/388`,{
                    'method':'POST',
                    'headers':{
                ...
                for (const jadwal of listJadwal) {
                    if(jadwal.nama_sub_tahap_siap === '') {
                        if(jadwal.set_tgl_cetak === null){
                            let postTanggal = await fetch(`https://burukab.sipd.kemendagri.go.id/siap/cetak-dpa/setting-link/daerah/main/budget/2022/388`,{
                                'method':'POST',
                                'headers':{
                                    'Content-Type':'application/x-www-form-urlencoded'
                                },
