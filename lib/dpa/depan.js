...
const download = async (listSKPD, listSKPDLengkap) => {
    if(!fs.existsSync(PATH.DPA.DEPAN)){
        console.log(`Membuat Folder ${PATH.DPA.DEPAN}`)
        fs.mkdirSync(PATH.DPA.DEPAN)
    } else {
        console.log(`Folder ${PATH.DPA.DEPAN} Sudah Ada`)
    }
    ...
    for (const [index, skpd] of listSKPDLengkap.entries()) {
        ...
        let file = `${PATH.DPA.DEPAN}\\${namaSKPD}.pdf`

        if(fs.existsSync(file)){
            fs.rmSync(file)
        }

        let res = ax.get(filter[0].link,{
            responseType:"stream"
        })
