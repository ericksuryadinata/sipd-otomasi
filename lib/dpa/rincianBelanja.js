...
const download = async (listSKPD) => {
    ...
    for (const [index, skpd] of listSKPD.entries()) {
        ...
        if(!fs.existsSync(skpdFolder)){
            ...
            console.log(`Folder ${skpdFolder} Sudah Ada`)
        }

        for (const link of listLink) {
            let namaKegiatan = link.namaKegiatan
            
            console.log(`melakukan download SKPD ${namaKegiatan} secara asynchronous`)

            let file = `${skpdFolder}\\${namaKegiatan}.pdf`
